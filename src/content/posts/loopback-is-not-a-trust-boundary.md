...
NVIDIA NemoClaw deploys OpenClaw in OpenShell Docker sandboxes. The container cannot reach `127.0.0.1` on the host. So NemoClaw starts Ollama with `OLLAMA_HOST=0.0.0.0:11434`. That bind is every interface. It is not loopback.

The installer prints `Using Ollama on localhost:11434`. The socket is not on localhost only.
